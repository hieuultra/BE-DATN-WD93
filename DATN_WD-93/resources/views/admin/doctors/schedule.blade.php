        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('admin.doctors.scheduleAdd') }}" method="POST">
                        @csrf
                        ...
                        <div class="mb-3">
                            ...
                            <div id="daySelection">
                                @php
                                use Carbon\Carbon;
                                $currentDate = Carbon::now()->startOfMonth(); // Bắt đầu từ ngày đầu tiên của tháng hiện tại
                                @endphp

                                @for ($monthOffset = 0; $monthOffset < 3; $monthOffset++)
                                    @php
                                    $dateForMonth=$currentDate->copy()->addMonths($monthOffset); // Lặp qua 3 tháng
                                    $daysInMonth = $dateForMonth->daysInMonth;
                                    @endphp

                                    <!-- Hiển thị tiêu đề cho từng tháng -->
                                    ...
                                    @for ($day = 1; $day <= $daysInMonth; $day++)
                                        ...
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input day-checkbox" type="checkbox" id="day{{ $date->format('Ymd') }}" name="days[]" value="{{ $dayOfWeek }} {{ $date->format('d/m/Y') }}" @if($isPast) disabled @endif data-date="{{ $date->format('Y-m-d') }}">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            editScheduleForm.addEventListener('submit', function(event) {
                ...
                axios.put(`/admin/doctors/doctor/scheduleUpdate/${scheduleId}`, formData)
                    .then(response => {
            ...
            deleteButtons.forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    if (confirm('Bạn có chắc chắn muốn xóa lịch làm việc này không?')) {
                        axios.delete(`/admin/doctors/doctor/scheduleDestroy/${scheduleId}`)
                            .then(response => {
