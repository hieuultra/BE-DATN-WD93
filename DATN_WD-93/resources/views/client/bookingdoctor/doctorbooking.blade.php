...
<body>
        ...
        <div class="container-fluid">
            ...
            <div class="collapse navbar-collapse">
                <ul class="navbar-nav ms-auto">
                    <li class="nav-item">
                        @if(Auth::check())
                        <a class="nav-link" href="{{ route('appointmentHistory', $user = Auth::user()->id) }}">Lịch sửa đặt khám</a>
                        @endif
                    </li>
                    <li class="nav-item">
                        @if(Auth::check())
                        <a class="nav-link" href="{{ route('admin.doctors.physicianManagement', $user = Auth::user()->id) }}">Bác sỹ quản lý</a>
                        @endif
                    </li>
                    <li class="nav-item">
    ...
    <div class="container mt-4">
        <div class="row mb-4 align-items-center">
            <div class="col-auto">
                <select id="locationFilter" class="form-select" onchange="filterDoctors()">
        ...
        <div class="doctor-card" data-location="{{ $doctor->user->address }}" data-name="{{ $doctor->user->name }}" style="margin-top: 10px;">
            ...
            <div class="doctor-info">
                ...
                <a class="text-primary" href="{{ route('doctorDetails', $doctor->id) }}">Xem thêm</a>
            </div>
            ...
            <div class="schedule">
                ...
                <div class="time-slot mt-3">
                    ...
                    <a href="{{ route('formbookingdt', $timeSlot->id) }}" style="text-decoration: none;">
                ...
                <p class="mt-2">
                    <i class="fas fa-hand-pointer"></i> Chọn và đặt (Phí đặt lịch {{ number_format($doctor->price, 0, ',', '.') }}đ)
                </p>
                <div class="address">
                    <strong>ĐỊA CHỈ KHÁM: {{ $doctor->user->address }}</strong>
        ...
        <script>
            ...
            function filterDoctors() {
                ...
                doctorCards.forEach(card => {
                    const doctorLocation = card.getAttribute('data-location');
                    const doctorName = card.getAttribute('data-name').toLowerCase();

                    const matchesLocation = locationFilter === 'all' || doctorLocation.includes(locationFilter);
